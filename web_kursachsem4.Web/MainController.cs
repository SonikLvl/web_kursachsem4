...
namespace web_kursachsem4.Web
{
    ...
    public class MainController : ControllerBase
    {
        ...
        // PUT /api/me/levels (Оновити рівні поточного користувача)
        [HttpPut("me/levels")]
        [Authorize]
        [ProducesResponseType(StatusCodes.Status204NoContent)] // Якщо оновлення пройшло успішно
        [ProducesResponseType(StatusCodes.Status400BadRequest)] // Наприклад, якщо тіло запиту null або валідація не пройшла
        [ProducesResponseType(StatusCodes.Status401Unauthorized)] // Забезпечується [Authorize]
        [ProducesResponseType(StatusCodes.Status404NotFound)] // Якщо користувача знайдено за токеном, але немає запису Level
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> EditMyLevels([FromBody] LvlsChangeRequest lvlsChangeRequest) // Додано [FromBody]
        {
            var userId = GetCurrentUserId();
            if (userId == null)
            {
                _logger.LogWarning("EditMyLevels: Unauthorized access attempt - no valid user ID in token.");
                return Unauthorized();
            }


            _logger.LogInformation("User {UserId} attempting to edit their levels with value: {LevelValue}.", userId.Value, lvlsChangeRequest?.Levels);

            // Додано перевірку RequestModel на null та валідацію ModelState
            if (lvlsChangeRequest == null || !ModelState.IsValid)
            {
                _logger.LogWarning("EditMyLevels: Invalid levels change request received.");
                return BadRequest(ModelState);
            }

            // Перевірка на null або порожній рядок
            if (string.IsNullOrEmpty(lvlsChangeRequest.Levels))
            {
                _logger.LogWarning("EditMyLevels: User {UserId} sent an empty or null level string.", userId.Value);
                return BadRequest("Level data cannot be null or empty.");
            }
            ...
            try
            {
                ...
                _logger.LogInformation("Levels for user ID {UserId} updated successfully.", userId.Value);
                return NoContent(); // 204 - успішно, без тіла відповіді

            }
            // Обробка NotFoundException з сервісу (якщо запис Level не знайдено для цього UserID)
            catch (NotFoundException nfex)
            {
                _logger.LogWarning(nfex, "EditMyLevels failed: User/level record not found for user ID {UserId}.", userId.Value);
                return NotFound(nfex.Message);
            }
            // Обробка ArgumentNullException з сервісу, якщо level string був null/порожній
            catch (ArgumentNullException anex)
            {
                _logger.LogWarning(anex, "EditMyLevels failed due to null/empty level string from service for user ID {UserId}.", userId.Value);
                return BadRequest(anex.Message); // Повертаємо 400 з повідомленням
            }
            // Обробка InvalidOperationException, якщо парсинг рівнів в GetLevelAsync мав би проблеми
            // (хоча цей catch для EditLevelAsync, але якщо сервіс EditLevelAsync міг би кинути такий виняток з якоїсь причини)
            catch (InvalidOperationException ioex)
            {
                _logger.LogError(ioex, "EditMyLevels failed due to unexpected operation error in service for user ID {UserId}.", userId.Value);
                return StatusCode(StatusCodes.Status500InternalServerError, "Error processing levels.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error editing levels for user ID {UserId}.", userId.Value);
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)] // Якщо користувача або його рівнів немає
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<bool[]>> GetMyLevels()
        {
            ...
            if (userId == null)
            {
                _logger.LogWarning("GetMyLevels: Unauthorized access attempt - no valid user ID in token.");
                return Unauthorized();
            }
            ...
            try
            {
                ...
                if (levels == null)
                {
                    _logger.LogInformation("Level data not found for user ID {UserId}.", userId.Value);
                    return NotFound("Level data not found.");
                }
                // Логування поверненого масиву може бути громіздким, краще логувати факт отримання
                _logger.LogInformation("Level data successfully retrieved for user ID {UserId}. Parsed into {Count} boolean values.", userId.Value, levels.Length);

                return Ok(levels); // Повертаємо масив bool[]
            }
            // Обробка InvalidOperationException з сервісу (якщо парсинг рядка рівнів не вдався)
            catch (InvalidOperationException ioex)
            {
                _logger.LogError(ioex, "GetMyLevels failed due to levels string parsing error for user ID {UserId}.", userId.Value);
                return StatusCode(StatusCodes.Status500InternalServerError, ioex.Message); // Повертаємо 500 з повідомленням про помилку парсингу
            }
            // Обробка NotFoundException, якщо сервіс кинув його (хоча GetLevelAsync повертає null)
            catch (NotFoundException nfex)
            {
                _logger.LogWarning(nfex, "GetMyLevels failed: User/level record not found for user ID {UserId}.", userId.Value);
                return NotFound(nfex.Message); // Цей catch може бути зайвим, якщо сервіс повертає null на 404
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting own levels for user ID {UserId}.", userId.Value);
